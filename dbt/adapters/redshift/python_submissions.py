...
class EmrServerlessJobHelper(PythonJobHelper):
    ...
    def __init__(
        self, parsed_model: Dict[Any, Any], credentials: RedshiftCredentials
    ) -> None:
        """
    ...
    def application_id(self) -> str:
        ...
        if app.get("emr_application_id", None):
            ...
            while not found:
                ...
                if apps:
                    app_id: str = next(
                        (app["id"] for app in apps if app["name"] == app_name), None
                    )
                if app_id:
                    found = True
    ...
    def application_ready(self) -> bool:
        ...
        while wait and not app_started:
            try:
                response = self.emrs_client.get_application(
    ...
    def save_compiled_code(self, compiled_code) -> str:
        ...
        table_name = (
            str(self.relation_name).replace(" ", "_").replace('"', "").replace(".", "_")
        )
        ...
        try:
            ...
            LOGGER.debug(
                f"Python compiled script uploaded to s3://{self.s3_bucket}/{s3_key}"
            )
...
"""
                    ...
                    LOGGER.debug(
                        f"Job: {job_run_id} completed for model: {self.relation_name}"
                    )
